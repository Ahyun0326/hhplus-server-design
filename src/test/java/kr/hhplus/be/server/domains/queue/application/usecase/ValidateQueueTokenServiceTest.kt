...
class ValidateQueueTokenServiceTest : BehaviorSpec({
    ...
    given("인증된 uuid에 매핑된 활성 토큰으로") {
        ...
        every { activeQueueRepository.isValidToken(scheduleId, token) } returns true
    ...
    given("다른 uuid에 매핑된 활성 토큰으로") {
        ...
        every { activeQueueRepository.findActive(scheduleId, uuid) } returns "other-token"
    ...
    given("uuid에 매핑된 토큰이 만료된 경우") {
        ...
        every { activeQueueRepository.findActive(scheduleId, uuid) } returns token
        every { activeQueueRepository.isValidToken(scheduleId, token) } returns false
